...
Feature: Verify order mode features
  ...
  @artifact_shopapp @mode_domestic @release_16L @priority_medium @project_UFT @use_regression @domain_customer @migrated_to_sdt
  Scenario: Verify that users see correct order date in order history page instead of 12/31/9999 for guest user
    Given I visit the web site as a guest user
    When I add an "available and orderable" product to my bag
    And I checkout until I reach the order confirmation page as a "guest" user
    And I should see order confirmation section displayed with order details
    ...
    When I navigate to order details page from footer
    And I lookup with order number and emailaddress in Order Status page
    Then I verify order date in OD page should not be out of range date

  @artifact_shopapp @mode_domestic @release_16L @priority_medium @project_UFT @use_regression @domain_customer @migrated_to_sdt
  Scenario:To verify that users see correct order date in order history page instead of 12/31/9999 for signed in user
    Given I visit the web site as a registered user
    When I add a "available and orderable" product to my bag
    And I checkout until I reach the order confirmation page as a "signed in" user
    And I capture order number from order_confirmation page
    And I navigate to order details page from footer
    Then I verify order date in OH page should not be out of range date

  @priority_medium @use_regression @mode_domestic @defect_AUTO-1304 @artifact_shopapp @shopapp_2 @domain_customer  @migrated_to_sdt
  Scenario: To verify display of text "Free" when shipping fee is zero in OD page
    Given I visit the web site as a guest user
    When I associate "free_ship_order" order in db
    And I navigate to the "order status" page from footer
    Then I verify order details for "delivered|DELIVERED" items in OH page
    And I should see shipping charge as free
  ...
  @artifact_shopapp @mode_domestic @release_16M @priority_medium @project_UFT @use_regression @domain_customer @migrated_to_sdt
  Scenario: Verify the message for the backorder item in order details page as a guest user
    Given I visit the web site as a guest user
    And I navigate to the "order status" page from footer
    And I search with below order number and email in order status page:
      |order_number| email          |
      |backordered | tdm@macys.com  |
  ...
  @priority_medium @use_regression @mode_domestic @artifact_shopapp @shopapp_2 @s4a_stable @domain_customer @migrated_to_sdt
  Scenario Outline:To verify the old order details page re-directions for mcom
    Given I visit the web site as a guest user
    When  I navigate with legacy url using order number "<order_number>", email address "<email>", area code "<area>", exchange number "<exchange>", and subscriber number "<subscriber>"
    Then  I should navigate to shopapp "<redirect_page>" page
    Examples:
      | order_number     | email                | area | exchange | subscriber | redirect_page |
      | free_ship_order  | TDM@MACYS.COM        |      |          |            | OD            |
      | free_ship_order  |                      | 998  | 937      | 6851       | OD            |
      | free_ship_order  |                      | 123  | 123      | 1234       | OH            |
      | free_ship_order  | sss@test.com         |      |          |            | OH            |
      | blank_order      | gerajend@in.ibm.com  |      |          |            | OH            |
      | blank_order      |                      | 601  | 904      | 5082       | OH            |
  ...
  Scenario:To verify the lock out error message for Email Address field
    ...
    Then I verify lockout error message after 4 attempts
      | We're still unable to locate your order. Please call our Customer Service Department at 1-800-BUY-MACY (1-800-289-6229), available 24 hours a day, 7 days a week. |
...
#Test : BOPS - processing for store pick-up - 27
  ...
  @artifact_shopapp @mode_domestic @release_16M @priority_medium @project_UFT @use_regression @domain_customer @migrated_to_sdt
  Scenario: Verify the message for the backorder item in OH and OD page as a signed in user
    Given I visit the web site as a guest user
    When I associate "backordered" order in db
    And I navigate to order history page
    Then I verify order details for "backordered|BACKORDERED" items in OH page
    When I navigate to the order details page as a signed in user
    Then I verify backordered text for backordered items in OD page
