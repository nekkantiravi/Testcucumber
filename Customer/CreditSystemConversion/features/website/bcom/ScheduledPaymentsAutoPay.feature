...
#       3.Feature will be Wrapped under Kill Switch: creditSchedulePaymentEnabled=true
  ...
  @wip @domain_customer @artifact_shopapp @project_otsp @release_
  Scenario Outline: Verify Autopay related deeplink for user with 'Autopayment' enabled of any kind
    Given I sign in with user having "<single_card>" added in profile
    When I navigate to the "credit services" page from footer
    Then I should see 'Manage Autopay' in deeplinks
    Examples:
      | single_card            |
      | single_card_sa_enabled |
      | single_card_pa_enabled |
      | single_card_ma_enabled |

  @wip @domain_customer @artifact_shopapp @project_otsp @release_
  Scenario Outline: Verify Autopay related deeplink for user when Scheduled Payment API' has failed
    Given I sign in with user having "<single_card>" added in profile with autopayment enabled
    When I navigate to the "credit services" page from footer
    Then I should see 'Manage Autopay' in deeplinks
    Examples:
      | single_card                     |
      | single_card_autopay_enabled     |
      | single_card_autopay_not_enabled |
