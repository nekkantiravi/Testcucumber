#####################################################################################
# Story             : B-66496 : T-N::UI::BCOM::Desktop::Disable Card Edit in Checkout for New Cobrand Card
...
#                 3. Feature be wrapped in kill switch( Kill switch = True ):  creditCvvSuppressionEnabled
  ...
  @wip @domain_customer @project_Checkout_First_Fourteen_Suppress_Security_Code_Field @release_16T @D @t
...
#####################################################################################
# Story             : B-66506: QE::T-N::MOCK::UI::BCOM::Desktop::Enable Card Edit in Checkout for New Cobrand Card if Account > 14 days old (TEST ONLY)
# Author            : Credit Systems Conversion Carryover :: QE Team
#####################################################################################

# Pre-Requisite:: 1. Customer applied for cobrand card and received a cobrand card online, and added it to her wallet via the Fusion acquisition flow.
#                 2. Card/Account should be more than 14 days
#                 3. Feature be wrapped in kill switch( Kill switch = TRUE):  creditCvvSuppressionEnabled

  @wip @domain_customer @project_Checkout_First_Fourteen_Suppress_Security_Code_Field @release_16T @e
