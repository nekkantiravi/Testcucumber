#####################################################################################################
# Story: B-92920 :: OTSP2: BUS:: MCOM:: My Account:: Display NON-Autopay + Autopay Scheduled Payment data
#        B-92924 :: OTSP2: BUS:: MCOM:: My Account:: "View details" link destination for Scheduled Payment use cases
# Author: QE Team
####################################################################################################

Feature: As a user with a card in profile, with a non-autopay or an autopay Scheduled Payment, I want to see that information on the MCOM My Account dashboard
...
#       		3.Feature will be Wrapped under Kill Switch: creditSchedulePaymentEnabled=true
  ...
  Scenario: User having single card with Autopay of unknown amount
    Given I sign in with user having "single_card_sa" added in profile
    When I navigate to my account page
  ...
  Scenario: User having single card with Autopayment is in process
    Given I sign in with user having "single_card_pa" added in profile
    When I navigate to my account page
  ...
  Scenario: User having single card with Multiple payments, of which one is an Autopayment, and the upcoming payment amount is KNOWN
    Given I sign in with user having "single_card_ma" added in profile
    When I navigate to my account page
  ...
  Scenario: User having single card with Multiple payments, of which one is an Autopayment, and the upcoming payment amount is UnKNOWN
    Given I sign in with user having "single_card_ma_unknown" added in profile
    When I navigate to my account page
