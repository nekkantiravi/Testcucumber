...
Feature: As an online credit customer, I would like to access my non dual line credit card(s) on the desktop version of the credit gateway page.
...
# Story NO: B-77788 : CM4:: BUS::UI::Desktop::MCOM::Comp-Credit Gateway Redesign::Home Page-Signed in
# Story NO: B-84609 : SDT: MCOM Desktop Automation for Signed IN Single / Multi Card view

  @domain_customer @project_responsive_gateway @release_17T
  ...
  Scenario Outline: Verify the elements display & its navigation on gateway responsive page for Signed User having dual line card
    Given I sign in with user having "dual_amex_user" added in profile
    When I navigate to the "credit services" page from footer
    And I click "<element>" on credit gateway page
    Then I should redirect to "<fusion_page>" page
    And I should see "<expected_process_indicator>" process indicator
  ...
  Scenario Outline: Verify the elements display & its navigation on gateway responsive page for Signed User having single amex card
    Given I sign in with user having "single_line_amex_user" added in profile
    When I navigate to the "credit services" page from footer
    And I click "<element>" on credit gateway page
    Then I should redirect to "<fusion_page>" page
    And I should see "<expected_process_indicator>" process indicator
  ...
  @domain_customer @project_responsive_gateway @release_17T
  Scenario: Verify the display of Credit Card FAQ's functionality on responsive gateway page for Signed User having Single or Multiple cards.
    Given I sign in with user having "new_prop_user" added in profile
    When I navigate to the "credit services" page from footer
  ...
  Scenario: Verify find store button on other ways to pay overlay when user having card in profile
    ...
    And I tap on "other_ways_to_pay_link"
    Then I should see other ways to pay overlay
    When I tap on "find_store_button"
  ...
  Scenario: Verify close icon on other ways to pay overlay when user having card in profile
    ...
    And I tap on "other_ways_to_pay_link"
    Then I should see other ways to pay overlay
    When  I tap on "close_other_ways_to_pay"
  ...
  Scenario: Verify suppression of links in quick tools section for Authorized user on credit gateway page
    Given I sign in with user having "authorized_user" added in profile
    When I navigate to the "credit services" page from footer
  ...
  @domain_customer @artifact_shopapp @project_responsive_gateway @release_17T
  Scenario: Verify right carousel functionality on responsive gateway page for profile having multi cards
    Given I sign in with user having "multi_card_user" added in profile
    When I navigate to the "credit services" page from footer
    Then I should see below elements in credit service gateway page:
      | right_carousel  |
      | left_carousel   |
  ...
  @domain_customer @artifact_shopapp @project_responsive_gateway @release_17T
  Scenario: Verify left carousel functionality on responsive gateway page for profile having multi cards
    Given I sign in with user having "multi_card_user" added in profile
    When I navigate to the "credit services" page from footer
    Then I should see below elements in credit service gateway page:
      | right_carousel  |
