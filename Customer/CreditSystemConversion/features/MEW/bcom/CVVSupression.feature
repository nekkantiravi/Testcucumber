#####################################################################################
# Story : B-66489 : QE::T-N::BCOM::MEW::Present/Require CVV2 field when Customer Checks Out with Cobrand Card in Wallet : Checkout Page (TEST ONLY)
# : B-66483 : QE::T-N::BCOM::MEW::Hide CVV2 field when Customer Checks Out with Cobrand Card in Wallet – Days 1-14 (TEST ONLY)
# : B-66475 : T-N::BCOM::MEW::Present CVV2 field and Message if Pre-Auth Unavailable
# : B-66477 : QE::T-N::BCOM::UI::MEW::Present CVV2 field with No Message if Kill Switch = False (TEST ONLY)
...
# Pre-Requisite:: Customer applied for cobrand card and received a cobrand card online, and added it to her wallet via the Fusion acquisition flow.
  ...
  @wip @domain_customer @project_Checkout_More_than_Fourteen_Require_Security_Code_Field @release_17C
  ...
  # Pre-Requisite:: Pre-auth service should be down and creditCvvSuppressionEnabled ks should be false
  @use_manual @domain_customer @project_Checkout_More_than_Fourteen_Require_Security_Code_Field @release_17D
  Scenario: No pre auth down message should be shown when KS is false
    Given I visit the mobile site as a registered user with new cobrand card added to wallet
    When I add an "available and orderable" product to my bag
    And I checkout until I reach the shipping & payment page as an "signed" user
    Then I should see CVV field is enabled
    And I should not see pre auth service down message
  ...
  Scenario: Verify that customer should see security code field and a message instructing to call customer service at checkout page
    ...
    ## Note:: Message should display as "Don't have your card yet? To checkout with this card, call Customer Service at 1-800-777-0000."
