Feature: Verify MWEDD 240 order confirmation 240 1I MWEDD email content

  @mwedd_fil_240_1I_order_confirmation @regression @mwedd_fil_240
  Scenario: Verify Email content for template mwedd_fil_240_1I_order_confirmation
    Given i trigger "mwedd_fil_240_1I_order_confirmation" input through csp testemail
  ...
  Scenario: Verify multi shipping methods scenario for template mwedd_fil_240_1C_order_confirmation
    Given i trigger "mwedd_fil_240_1I_order_confirmation_multi_shipping_methods" input through csp testemail
    And i have an enhanced payload sent to email provider
    When i navigate to view the email page
  ...
  Scenario: Verify paypal payment method scenario for template mwedd_fil_240_1C_order_confirmation
    Given i trigger "mwedd_fil_240_1I_order_confirmation_paypal_payment_method" input through csp testemail
    And i have an enhanced payload sent to email provider
    When i navigate to view the email page
    Then i should see Macys logo
