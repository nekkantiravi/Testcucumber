Feature: Verify MWEDD 264 bops ready for pickup email content
  ...
  @mwedd_fil_264_14S_bops_ready_for_pickup @regression
  Scenario: Verify Email content for for template mwedd_fil_264_14S_bops_ready_for_pickup
    Given i trigger "mwedd_fil_264_14S_bops_ready_for_pickup" input through csp testemail
    ...
    And i should see body text for 264_14S and 14T:
      """
      Good news! <billingAddr.firstName>'s registry order from Macys.com is ready for pick up. You were designated as the person to pick it up.
      """
    ...
    And i should see what to bring details
      """
      What to bring:• A government-issued photo ID• This email with order # and barcode below
      """
