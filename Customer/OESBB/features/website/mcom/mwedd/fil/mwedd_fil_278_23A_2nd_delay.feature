Feature: Verify MCOM 278 2nd delay email content
  ...
  @mwedd_fil_278_23A_2nd_delay @regression
  Scenario: Verify Email content for for template mwedd_fil_278_23A_2nd_delay
    Given i trigger "mwedd_fil_278_23A_2nd_delay" input through csp testemail
    ...
    #We're sorry, but item(s) from order #<ordernumber> are further delayed with the vendor. At this time, the vendor is unable to provide a new ship date.
		#Note:We need to hear from you to confirm you're willing to wait for the item(s). Simply reply to this email or call us at 1-800-BUY-MACY (1-800-289-6229). Otherwise, we'll have to cancel your backorder after 30 days.
