Feature: Verify MCOM 245 virtual return receipt email content

 @regression
  Scenario: Verify email content for maf_245_5L_MCOM template
    Given i trigger "mcom_maf_245_5L_virtual_return_receipt" input through csp testemail
  ...
  @optional
  Scenario: Verify optional fields empty scenario for template mcom_maf_245_5L_virtual_return_receipt
    Given i trigger "mcom_maf_245_5L_virtual_return_receipt_optional" input through csp testemail
    And i wait for enhanced payload to be sent to email provider
    When i navigate to view the email page
    Then i should see Macys logo
    
  @production
  Scenario: Production : Verify email template for mcom_maf_245_5L_virtual_return_receipt
    Given i trigger "mcom_maf_245_5L_virtual_return_receipt" input through csp testemail    
    When i navigate to view the email page
    Then i should see Macys logo