Feature: Verify MCOM 542 bops order late pickup cancel email content
  ...
  @mcom_fil_542_30C_bops_order_late_pickup_cancel @regression
  Scenario: Verify Email content for for template mcom_fil_542_30C_bops_order_late_pickup_cancel
    Given i trigger "mcom_fil_542_30C_bops_order_late_pickup_cancel" input through csp testemail
    ...
    And i should see body text for bops order late pickup cancel:
      """
      We're sorry, the final pick-up date has passed and your order is no longer available at <storename>. We'll issue a refund on the payment method listed below. If you assigned a pick-up contact, they'll receive an email, too. You may still find the items on macys.com or at another Macy's store. Check the product links below to place a new order.
      """
   ...
   @optional
   Scenario: Verify optional fields empty scenario for template mcom_fil_542_30C_bops_order_late_pickup_cancel
    Given i trigger "mcom_fil_542_30C_bops_order_late_pickup_cancel_optional" input through csp testemail
    When i navigate to view the email page
