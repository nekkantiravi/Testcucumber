Feature: Verify email content for 245_return_confirmation template
  ...
  @mcom_fil_245_5A_return_confirmation @regression
  Scenario: Verify Email content
    Given i trigger "mcom_fil_245_5A_return_confirmation" input through csp testemail
    And i have an enhanced payload sent to email provider
    When i navigate to view the email page
    And i should see pre header:
      """
      Thanks for returning your item(s). Shop Macy's
      """
   ...
   @optional
   Scenario: Verify optional fields empty scenario for template mcom_fil_245_5A_return_confirmation
    Given i trigger "mcom_fil_245_5A_return_confirmation_optional" input through csp testemail
    When i navigate to view the email page
