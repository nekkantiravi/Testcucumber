Feature: Verify MCOM 240 bops mixed bag order confirmation email content

  @mcom_fil_240_1P_bops_mixed_bag @regression
  Scenario: Verify Email content for for template mcom_fil_240_1P_bops_mixed_bag
    Given i trigger "mcom_fil_240_1P_bops_mixed_bag" input through csp testemail
    ...
    And i should see body text:
      """
      Thanks for shopping at Macy's! Once your order is processed, we'll email details on the item(s) being shipped and item(s) being picked up in store. Don't worry, you won't be charged until then.
      """
	...
	@optional
   Scenario: Verify optional fields empty scenario for template mcom_fil_240_1P_bops_mixed_bag
    Given i trigger "mcom_fil_240_1P_bops_mixed_bag_optional" input through csp testemail
    When i navigate to view the email page
