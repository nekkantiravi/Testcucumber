Feature: Verify MCOM 246 gift return email content
  ...
  @246_gift_return
  @mcom_fil_246_6A_gift_return  @regression
  Scenario: Verify Email content for for template mcom_fil_246_6A_gift_return
    Given i trigger "mcom_fil_246_6A_gift_return" input through csp testemail
    ...
    Then i should see body text:
      """
      hi oes, we're processing your credit for the returned item(s). you'll receive a macy's gift card by mail in the amount listed below. please note: the macy's gift card can be used to purchase merchandise online and in store; it isn't redeemable for cash. thanks for shopping at macy's! 
      """
    ...
    And i should see product color for gift return
    And i should see product price
