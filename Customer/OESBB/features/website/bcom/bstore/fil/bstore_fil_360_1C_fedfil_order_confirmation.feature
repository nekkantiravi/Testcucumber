Feature: Verify fedfil order confirmation templates
  ...
  @bstore_fil_360_1C_fedfil_order_confirmation 
  @regression
  @bstore_fil_360
  Scenario: Verify email content for template bstore_fil_360_1C_fedfil_order_confirmation
    Given i trigger "bstore_fil_360_1C_fedfil_order_confirmation" input through csp testemail
    ...
    @regression
    Scenario: Verify Email content for template bstore_fil_360_1C_fedfil_order_confirmation_apple_payment
    Given i trigger "bstore_fil_360_1C_fedfil_order_confirmation_apple_payment" input through csp testemail
    And i have an enhanced payload sent to email provider
    When i navigate to view the email page
    And i should see bloomingdales logo
    And i should see freeshipping image
