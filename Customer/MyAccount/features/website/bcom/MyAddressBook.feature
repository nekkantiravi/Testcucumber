...
Feature: To verify add shipping address in My address book page
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_high @use_regression
  @myaccount_5 @s4a_stable @domain_customer @migrated_to_sdt
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_medium @use_regression
  @myaccount_5 @s4a_stable @domain_customer @migrated_to_sdt
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_low @use_regression
  @myaccount_5 @s4a_stable @domain_customer @migrated_to_sdt
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_medium @use_regression
  @myaccount_5 @s4a_stable @domain_customer @saucelab_M @saucelab_M_F1 @migrated_to_sdt
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_medium @use_regression
  @14H @myaccount_5  @s4a_stable @domain_customer @migrated_to_sdt
  Scenario:Verify error messaging when adding an address with invalid mandatory fields on My Address Book Page
    ...
    Then I verify error message while adding address with invalid data or missing field on my address book page
      | field_name       | value  | bcom_error_message                    |
      ...
      | address_state    | Select | Please select a state.                |
      | address_zip_code | !@#$%  | Please enter a valid zip code.        |
  ...
  Scenario:Verify messaging on a successful deletion of address on My Address Book Page
    ...
    Then I should see this message:
      | An entry has been deleted from your address book. |
  ...
  Scenario:Verify Set As Primary is disabled for a primary address on My Address Book Page
    Given I visit the web site as a guest user
    When I create a new profile
    And I add 2 addresses to my address book
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_high @use_regression
  @myaccount_5 @s4a_stable @domain_customer @use_domain_qual @cm_dsv_high_10 @migrated_to_sdt
  Scenario:Verify Address Book functionality - Add, Edit, Remove and Make Primary
    Given I visit the web site as a registered user
    When I add 1 addresses to my address book
  ...
  @upi_88_mcom @upi_100_bcom @artifact_shopapp @priority_medium @use_regression
  @myaccount_5 @s4a_stable @domain_customer @migrated_to_sdt @run_this
