Feature: Verify Easy Return
  ...
  @returns_regression @14K @use_regression @mode_domestic @credit-7386 @artifact_shopapp @returns_mgt_3 @domain_customer @migrated_to_sdt
  Scenario Outline: Customer help pages redirect correctly to the easy returns page
    Given I visit the web site as a guest user
    When I navigate to following customer "<MCOM_URL>" page url
    Then I verify customer page url navigation with "<navigation_type>" page
    Examples:
      | navigation_type  | MCOM_URL                                                                 |
      | app              | https://macys--tst.custhelp.com/app                                      |
      | app_answers_list | https://macys--tst.custhelp.com/app/answers/list/c/4                     |
      | app_mail         | https://macys--tst.custhelp.com/app/answers/detail/a_id/140#returnbymail |
  ...
  Scenario: Error message validations using invalid data on the Easy Returns page as a Guest user
    ...
    Then I verify lockout error message after 4 attempts
      | We're still unable to locate your order. Please call our Customer Service Department at 1-800-BUY-MACY (1-800-289-6229), available 24 hours a day, 7 days a week. |
    When I lookup with "invalid" order on EasyReturns page
