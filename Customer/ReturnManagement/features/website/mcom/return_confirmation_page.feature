Feature:  Verify Return Confirmation Page
  ...
  @returns_regression @15G @credit-5774 @use_project @mode_domestic @priority_high @artifact_shopapp @returns_mgt_5 @shopapp_3 @domain_customer @migrated_to_sdt
  Scenario: Print mailing label button is correct on the return confirmation page
  ...
  Scenario: Estimated Credit is displayed correctly on the Return Submit page as a Guest user
    Given I visit order history page as a guest user
    When I navigate to confirmation page using "shipping and billing different" order as a "guest" user
    Then I should see estimated credit as per the credit section


  @priority_high @artifact_shopapp @shopapp_4 @domain_customer @use_regression
...
########################################################################################################################
  ...
  Scenario: Return Instructions should be shown on the return confirmation page when an email is used as a Signed in User
    ...
    Then I should see following return instruction in confirmation page on each site:
      |Print your mailing label and attach it to your return package. Don't forget to include this confirmation page. We'll also email a copy of your label and tracking number to <email_address>|
