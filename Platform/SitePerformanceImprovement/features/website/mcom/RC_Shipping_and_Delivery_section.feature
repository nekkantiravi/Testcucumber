Feature: Regression scenarios for Shipping and Delivery section of Responsive Checkout page

@domain_Site_performance_Optimization @guest_checkout @Rc_Shipping_Delivery
...
|Macy's American Express|
  ...
  @domain_Site_performance_Optimization @guest_checkout @Rc_Shipping_Delivery
  Scenario: Verify the selected shipping method displayed in shipping & delivery section for guest user
    Given I visit the website as a guest user using rest services
    ...
    And I enter details on the shipping section for "guest" and continue from shipping section
    And I select the shipping method in shipping method section for "guest" user
    Then I verify that the choosen shipping method displayed in shipping & delivery section for "guest" user

  @domain_Site_performance_Optimization @Registered_checkout @Rc_Shipping_Delivery
  Scenario: Verify the selected shipping method displayed in shipping & delivery section for signedIn user
    ...
    And I select the shipping method in shipping method section for "registered" user
    Then I verify that the choosen shipping method displayed in shipping & delivery section for "registered" user

  @domain_purchase_and_delivery @signedin @checkout
  Scenario: Verify the default shipping method selected in shipping & delivery section for guest user
    Given I visit the website as a guest user using rest services
    When I add an "available and orderable" product to my bag using rest service that is not "registrable and available_bops and BT_furniture"
    And I checkout until I reach the shipping & payment page as a "guest" user
    And I enter details on the shipping section for "guest" and continue from shipping section
    Then I verify that first shipping method is selected by default for "guest" user
    And I verify selected shipping method displays in order section for "guest" user
  ...
  Scenario: Verify the default shipping method selected in shipping & delivery section for guest user
    ...
    And I checkout until I reach the shipping & payment page as a "registered" user
    And I enter details on the shipping section for "registered" and continue from shipping section
    Then I verify that first shipping method is selected by default for "registered" user
    And I verify selected shipping method displays in order section for "registered" user
