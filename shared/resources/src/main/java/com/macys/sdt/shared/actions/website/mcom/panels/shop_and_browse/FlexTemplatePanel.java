...
public class FlexTemplatePanel extends StepUtils {

    private static final Logger logger = LoggerFactory.getLogger(FlexTemplatePanel.class);
    List<WebElement> rowElements = new ArrayList<>();
    public static WebElement mediaExistsRowElement = null;

    public boolean isMediaExistsOnPage(String mediaName, String rowType){
        logger.info("Media Name:"+mediaName+" and RowType:"+rowType+" lookup initiated");
        if (mediaName.equalsIgnoreCase("copy_block") && rowType.equals("101")) {
            Wait.untilElementPresent("category_splash.copy_block_section");
            return Elements.elementPresent("category_splash.copy_block_section");
        }
        boolean mediaExistence = false;
        ...
        String xpathIdentifier = edge() ? ".//ul" : ".//ul[contains(@class,'-block-grid-')]";
        Assert.assertFalse("ERROR - DATA: Media rows are not exists on URL"+ WebDriverManager.getCurrentUrl(), rowElements.isEmpty());
        Iterator rowElementsIterator = rowElements.iterator();
        while (rowElementsIterator.hasNext()){
            WebElement rowElement = (WebElement)rowElementsIterator.next();
            if((mediaName.equalsIgnoreCase("ad") || mediaName.equalsIgnoreCase("image_map")) && (rowType.equals("0") || rowType.equals("101"))) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/img"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div/img"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div/a/img"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    String mapValue = adMediaList.get(0).getAttribute("usemap");
                    if (mediaName.equalsIgnoreCase("ad") && mapValue == null) {
                        mediaExistence = true;
                        mediaExistsRowElement = rowElement;
                        break;
                    } else if (mediaName.equalsIgnoreCase("image_map") && mapValue != null) {
                        mediaExistence = true;
                        mediaExistsRowElement = rowElement;
                        break;
                    }
                }
            } else if (mediaName.equalsIgnoreCase("product_panel_category") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div//span[contains(@class,'shopAll')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div//span[contains(@class,'shopAll')]"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if (mediaName.equalsIgnoreCase("video") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//map/area")).stream().filter(f-> f.getAttribute("data-video-component-id") != null && !f.getAttribute("data-video-component-id").equals("")).collect(Collectors.toList());;
                if(adMediaList.isEmpty())
                     adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div/map/area")).stream().filter(f-> f.getAttribute("data-video-component-id") != null && !f.getAttribute("data-video-component-id").equals("")).collect(Collectors.toList());
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"//a[contains(@href, '/video/')]"));
                if (!adMediaList.isEmpty()) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if ((mediaName.equalsIgnoreCase("thumbnail_grid") || mediaName.equalsIgnoreCase("media_ads")) && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//div[contains(@id, 'filters')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div[contains(@class, 'sortableGrid')]"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    if(mediaName.equalsIgnoreCase("media_ads")){
                        if(adMediaList.get(0).findElements(By.xpath(xpathIdentifier+"//li/div[contains(@class, 'media-component')]")).size() >= 1){
                            mediaExistence = true;
                            mediaExistsRowElement = rowElement;
                            break;
                        }
            ...
            }else if (mediaName.equalsIgnoreCase("horizontal_rule") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div/hr"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div/hr[contains(@class, 'canvasRule')]"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if (mediaName.equalsIgnoreCase("text") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//span[contains(@class, 'textComponent')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div//*[contains(@class, 'textComponent')]"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if (mediaName.equalsIgnoreCase("slideshow") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//div[contains(@id, 'bSlide')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li/div//li[contains(@id, 'bSlide')]"));
                if (!adMediaList.isEmpty()) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if (mediaName.equalsIgnoreCase("flexible_pool") && rowType.equals("101")) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//div[contains(@class, 'flexPool')]//span[contains(@class, 'shopAll')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.className("flexPool"));
                if (!adMediaList.isEmpty() && adMediaList.size() == 1) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else if (mediaName.toLowerCase().contains("category_icon") && (rowType.equals("102") ||rowType.equals("103") || rowType.equals("104") || rowType.equals("105") || rowType.equals("106"))) {
                List<WebElement> adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//div[contains(@class, 'adCatIcon')]"));
                if(adMediaList.isEmpty())
                    adMediaList = rowElement.findElements(By.xpath(xpathIdentifier+"/li//div[contains(@class, 'adCatIcon')]"));
                if (!adMediaList.isEmpty() && ((rowType.equals("102") && adMediaList.size() == 2) || (rowType.equals("103") && adMediaList.size() == 3) || (rowType.equals("104") && adMediaList.size() == 4) || (rowType.equals("105") && adMediaList.size() == 5) || (rowType.equals("105") && adMediaList.size() == 5) || (rowType.equals("106") && adMediaList.size() == 6))) {
                    mediaExistence = true;
                    mediaExistsRowElement = rowElement;
                    break;
                }
            }else {
                Assert.fail("Please check given parameters values: mediaName='"+mediaName+"' and rowType='"+rowType);
            }
        }
        logger.info("Media Name:"+mediaName+" and RowType:"+rowType+" lookup completed");
        return mediaExistence;
    }

    public Map getMediaDetailsOnPage(String mediaName, String rowType){
        if(mediaExistsRowElement == null){
            ...
            Assert.assertTrue(errorMessage, isMediaExistsOnPage(mediaName,rowType));
            Assert.assertFalse(errorMessage, mediaExistsRowElement == null);
        }
        Map imageAndLinkData = new HashMap<>();
        imageAndLinkData.put("imageSrcs", mediaExistsRowElement.findElements(By.tagName("img")).stream().map(m -> m.getAttribute("src")).collect(Collectors.toList()));
        imageAndLinkData.put("linkUrls", mediaExistsRowElement.findElements(By.tagName("a")).stream().map(m -> m.getAttribute("href")).collect(Collectors.toList()));
        imageAndLinkData.put("linkTexts", mediaExistsRowElement.findElements(By.className("adFlexText")).stream().map(m -> m.getText()).collect(Collectors.toList()));
        return imageAndLinkData;
    }


    public String getPageSignatureAttribute(String attributeName){
        ...
        return siteInfo.select("appserver").html();
    }
    ...
    public List getSequenceByRowTypes(String rowType) {
        ...
        seqNumbers = rowElements.stream()
                .filter(element -> element.getAttribute("id").contains("row_"))
                .map(element -> element.getAttribute("id").split("_")[2])
                .collect(Collectors.toList());
//        seqNumbers = rowElements.stream()
//                .filter(element -> element.getAttribute("id").contains("row_" + rowType))
//                .map(element -> element.getAttribute("id").split("_")[element.getAttribute("id").split("_").length - 1])
//                .collect(Collectors.toList());
        return seqNumbers;
    }
    ...
    public List<WebElement> rowElements() {
        Wait.untilElementPresent(macys() ? "flexible_template.global_container" : "flexible_template.media_container");
        WebElement rowContainer= Elements.findElement(macys() ? "flexible_template.global_container" : "flexible_template.media_container");
        List<WebElement> rows = rowContainer.findElements(By.xpath("//div[contains(@id, 'row_')]" ));
        return rows;
    }
    ...
    public String selectMediaLinksByRowTypeSeqNumber(String rowType, String seqNumber) {
        String rowTypeId = "row_" + rowType + "_" + seqNumber;
        WebElement rowElement = getRowElement(rowTypeId);
        List<WebElement> mediaLinks = rowElement.findElements(By.tagName("a")).stream().map(e -> e).collect(Collectors.toList());
        Random random = new Random();
        int index = random.nextInt(mediaLinks.size());
        String href = mediaLinks.get(index).getAttribute("href");
        Clicks.click(mediaLinks.get(index));
        return href;
    }
    ...
    public Map getZoneMedia(String rowType, String rowTypeId, boolean exceptEmptyRow, boolean c2Flag, String zoneId) {
        ...
        mediaType = getMediaTypeByRow((Arrays.asList(oneRows).contains(rowType) ? getRowElement(rowTypeId) : getRowZoneElement(rowTypeId, zoneId)), exceptEmptyRow);
        //        if (Arrays.asList(oneRows).contains(rowType)) {
        ...
        if (c2Flag) {
            mediaData.put("mediaInfo",
                    getMediaValues((Arrays.asList(oneRows).contains(rowType) ? getRowElement(rowTypeId) : getRowZoneElement(rowTypeId, zoneId)), mediaType, exceptEmptyRow, c2Flag));
        }
        //        if (c2Flag) {
        //            if (Arrays.asList(oneRows).contains(rowType)) {
        ...
        switch (mediaType) {
            ...
            case "slideshow":
                if (c2Flag) {
                    ...
                    slideshowImages = rowElement.findElement(By.className("slideContainer")).findElements(By.className("bannerSlides")).stream()
                            .map(element -> element.findElement(By.tagName("img")).getAttribute("src"))
                            .map(img -> img.split("/")[img.split("/").length - 1])
                            .collect(Collectors.toList());
    ...
    public Map getImageAndLinkDetails(String mediaClassName){
        Map imageAndLinkData = new HashMap<>();
        List<WebElement> requiredRowElements = Elements.findElements(By.className(mediaClassName));
        ArrayList<String> linkData = new ArrayList<>();
        ArrayList<String> imageData = new ArrayList<>();
        ArrayList<String> linkTextData = new ArrayList<>();
        requiredRowElements.forEach(rowEle -> {
            if (mediaClassName.equalsIgnoreCase("iconTextLarge")) {
                WebElement linkText = rowEle.findElement(By.className("adFlexText"));
                if (linkText != null && linkText.isDisplayed())
                    linkTextData.add(linkText.getText());
            }
            if (mediaClassName.equalsIgnoreCase("adCatIcon") || mediaClassName.equalsIgnoreCase("flexPool") || mediaClassName.equalsIgnoreCase("rowType101") || mediaClassName.equalsIgnoreCase("rowType0")) {
                WebElement ImageEle = mediaClassName.equalsIgnoreCase("rowType101") || mediaClassName.equalsIgnoreCase("rowType0") ? rowEle.findElement(By.xpath("//div[@class=\"columned\"]//ul/li//img")) : rowEle.findElement(By.tagName("img"));
                if (ImageEle != null && ImageEle.isDisplayed())
                    Clicks.hoverForSelection(ImageEle);
                    Wait.ajaxDone();
                    Wait.animationDone();
                    imageData.add(ImageEle.getAttribute("src"));
            }
            if (mediaClassName.equalsIgnoreCase("adCatIcon") || mediaClassName.equalsIgnoreCase("flexPool") || mediaClassName.equalsIgnoreCase("iconTextLarge")) {
                WebElement linkEle = rowEle.findElement(By.tagName("a"));
                if (linkEle != null && linkEle.isDisplayed())
                    linkData.add(linkEle.getAttribute("href"));
            }
        });
        imageAndLinkData.put("linkUrls",linkData);
        imageAndLinkData.put("imageSrcs", imageData);
        imageAndLinkData.put("linkTexts", linkTextData);
        return imageAndLinkData;
    }
    ...
    public WebElement getRowElement(String rowId) {
        ...
        rowElement = rowElements.stream().filter(element -> element.getAttribute("id").equals(rowId)).findFirst().get();
        //        for (WebElement element : rowElements) {
        //            if (element.getAttribute("id").equals(rowId)) {
        //                rowElement = element;
        //            }
        //        }
        Assert.assertFalse("Element not displayed on page with id: " + rowId, rowElement == null);
        //        if (rowElement == null)
    ...
    public boolean popupVideoExists(WebElement rowElement){
        boolean exists = false;
        if (imageMapExists(rowElement)){
            return rowElement.findElements(By.tagName("area")).stream().anyMatch(e -> e.getAttribute("data-video-component-title") != null);
        }
        return exists;
    }
    ...
    public boolean productPoolExists(WebElement rowElement) {
        boolean exists = false;
        exists = (rowElement.findElements(By.className("componentCarousel")).size() > 0) || (rowElement.findElements(By.className("productThumbnail")).size() > 0);
        return exists;
    }
    ...
    public boolean flexibleLinksExists(WebElement rowElement) {
        boolean exists = false;
        exists = rowElement.findElements(By.className("flexLinks")).size() > 0;
        return exists;
    }
    ...
    public boolean productPanelPoolExists(WebElement rowElement) {
        boolean exists = false;
        exists = rowElement.findElements(By.className("shopAll")).size() > 0 ? rowElement.findElement(By.className("shopAll")).getText().contains("Shop All") : false;
        return exists;
    }
    ...
    public String getMediaTypeByRow(WebElement rowElement, boolean exceptEmptyRow) {
        ...
        if (bannerMachineSlideshowExists(rowElement)) {
            ...
            mediaType = "widget";
        } else if (popupVideoExists(rowElement)) {
            mediaType = "video";
        ...
        } else if (flexibleLinksExists(rowElement)) {
            mediaType = "flexible_links";
