...
Feature: Customer's contact history,status,date,and title are properly displayed.
  ...
  Scenario: I mark all todos as completed
  ...
  Scenario: Customer's contact history,status,date,and title columns are displayed in History Contact tab
    ...
    And I type the name of a customer "KAREN" in the search box
  ...
  Scenario: After creating a ToDo we check that the SA name is displayed, title is truncated after 20 characters, status
    ...
    And I input client name "KAREN BAKERTON" in the input filed from MY TO DOS page
    And I select the full name "KAREN BAKERTON" from client dropdown MY TO DOS page
    And I enter a description "KAREN This is a regression test" in the input field on MY TO DOS page
    ...
    And the NAME of the SA "ONE, FIFTY" that created the TO DO is displayed MY TO DOS page
