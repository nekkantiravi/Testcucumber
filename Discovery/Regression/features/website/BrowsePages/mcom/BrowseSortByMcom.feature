Feature: SortBy verification on Category Browse Page

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page @discovery_daily_run
  Scenario Outline: CategoryBrowsePage - Domestic|Iship|Registry - Verify facet selection persistence with 'sort by' option
    Given I am on CategoryBrowsePage for "<category_id>" category id in <shopping_mode> mode
    When I select 'multiple' facet value from 'any' facet sections
    Then I verify that products are filtered as per selected facet values
    When I select "New Arrivals" in sort by drop down
    And I select random product from thumbnail grid
    Then I verify that landed on respective product display page
    When I select browse 'back' button
    Then I verify that landed on CategoryBrowsePage on same product grid point
    When I navigate to top of page
    Then I verify that previously selected facets persists in breadcrumb
    And I verify that previous sort by selection persist
    Examples:
      | shopping_mode | category_id |
      | Domestic      | 3310        |
      | Registry      | 7552        |
      | Iship         | 11221       |
    # Notes:
    # Verify facet selection persists when we navigate back from PDP page

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page @xbrowser_browse
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality: Price: High to Low
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    And I select "Price: High to Low" in sort by drop down
    Then I verify that the Sort By "Price: High to Low" functionality

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality: Price: Low to High
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    And I select "Price: Low to High" in sort by drop down
    Then I verify that the Sort By "Price: Low to High" functionality

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    Then I verify that the Sort By displayed with all options
   # Notes:
   # Verify all sort by options are displayed

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality: Customer Top Rated
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    Then I verify that the product count is displayed
    When I select "Customers' Top Rated" in sort by drop down
    Then I verify that the Sort By "Customers' Top Rated" functionality
   # Notes:
   # Verify that products are displayed with non broken images and links, price and quick peek links

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality: Best Sellers
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    Then I verify that the product count is displayed
    When I select "Best Sellers" in sort by drop down
    Then I verify that the Sort By "Best Sellers" functionality

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: priority_medium - Domestic - Verify Sort By functionality: New Arrivals
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    Then I verify that the product count is displayed
    When I select "New Arrivals" in sort by drop down
    Then I verify that the Sort By "New Arrivals" functionality

  @domain_discovery @mode_domestic @mode_registry @mode_iship @snbc_comp @use_regression @feature_browse_page
  Scenario: CategoryBrowsePage - Domestic - Verify Sort By functionality: Featured Items
    Given I am on CategoryBrowsePage for "jeans" under "Men" in Domestic mode
    Then I verify that the product count is displayed
    When I select "Featured Items" in sort by drop down
    Then I verify that the Sort By "Our Top Picks" functionality

  @use_regression @artifact_navapp @domain_discovery @priority_high @snbc_comp @use_regression @feature_browse_page
  Scenario Outline: BrowsePage - - Verify Sort By
    Given I am on CategoryBrowsePage for "<Category_id>" category id in Domestic mode
 #Given I am on Browse Page for "<sub category>" under "<category>"
    Then I verify that the Sort By displayed with all options
    Examples:
      | Category_id |
      | 55971       |
      | 9915        |
      | 269         |
      | 55637       |
      | 32918       |
      | 48561       |
      | 60600       |
      | 17570       |
      | 51906       |
      | 55352       |
      | 57386       |
    # Notes:
    # Verify all sort by options are displayed