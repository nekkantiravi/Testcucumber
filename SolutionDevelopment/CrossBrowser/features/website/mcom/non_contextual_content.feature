...
#      | 101      | WIDGET            |
  ...
  Scenario Outline: Category Splash Page -  Verify row101 are contextualized on desktop in REGISTRY mode for non contextual media
  ...
  Scenario Outline: Browse page - Verify row101 are contextualized on desktop in DOMESTIC mode for non contextual media
    Given I visit the web site as a guest user
...
#      | 101      | WIDGET                 |
  ...
  Scenario Outline: Browse page - Verify row101 are contextualized on desktop in ISHIP Mode for non contextual media
    Given I visit the web site as a guest user in "iship" mode
    When I navigate to "Browse" category with "<media>" in "<row_type>" for context
      | DEVICE_TYPE | REGION_CODE | SHOPPING_MODE | NAVIGATION_TYPE |
      | DESKTOP     | INTL        | SITE          | BROWSE          |
