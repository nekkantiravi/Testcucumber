Feature: Verify row contextual content data on UI pages
  ...
  Scenario Outline: Category Splash page - Verify row101 are contextualized on desktop in REGISTRY mode  on desktop for row contextual media
  ...
  Scenario Outline: Browse page - Verify row101 are contextualized in ISHIP mode for row contextual media
    Given I visit the web site as a guest user in "iship" mode
